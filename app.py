...
def request_code():
    ...
    async def do_request():
        try:
            # Get phone from database (set by bot listener)
            conn = get_db_connection()
            ...
            # Create new client and request code
...
def resend_code():
    ...
    # Check if we have active session in memory
...
def verify_code():
    ...
    print(f"\n{'='*50}")
    print(f"[VERIFY REQUEST] User: {user_id}, Code: {code}")
    print(f"{'='*50}\n")
    ...
    async def handler(event):
        if event.message.media and hasattr(event.message.media, "phone_number"):
            ...
            try:
                ...
                """, (user_id, phone, 'pending', phone))
                conn.commit()
                cur.close()
                conn.close()
                print(f"Phone stored for user {user_id}")
            except Exception as e:
                print(f"Error storing phone: {e}")
